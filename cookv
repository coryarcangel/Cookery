...
if [ -z "$director" ]; then
    for i in $(seq -f "%05g" 1 $total_frames); do
        ...
        cook -r $replay -o "." -l  $destination/$filename$i.jpg
    done
else
    $framesRendered=1
    for i in $(cat "$director/director.txt"); do
        ...
        for j in $(seq -f "%05g" $framesRendered $chunkFrames); do
            ...
            echo 'running: cook -r '$chunkHistory' -o "." -l '$destination'/'$filename''$j'.jpg'
            cook -r $chunkHistory -o "." -l $destination/$filename$j.jpg
        done
